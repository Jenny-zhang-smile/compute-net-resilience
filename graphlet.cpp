...
void generate_orca_file(int organism, int instance, string input, bool simple_output, vector<string> &proteins) {
	cout << ORCA_FILE_DIR << endl;
	cout << get_output_path(ORCA_FILE_DIR, organism, simple_output).c_str() << endl;
...
void run_orca(int organism, int graphlet_size, bool simple_output) {
	...
	cout << "run_orca " << endl;
	cout << ORCA_FILE_DIR << endl; //orca-data
	cout << ORCA_STAT_DIR << endl; //orca-statistics
	cout << ORCA_LOG_DIR << endl;
	
    string orca_exec = simple_output ? "./orca.exe" : ORCA_EXEC_LOCATION;
