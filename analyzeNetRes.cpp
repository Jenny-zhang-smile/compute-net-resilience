...
void analyze_organism(int organism, string input, bool simple_output,int rates) {
	...
	int i=0;int j=0; double sum=0; int maxcout=100;
	PGraph H;

    PGraph G = create_graph(organism, input, proteins);
	
	ofstream staticfile("static0",ios::app);  
	ofstream resaverage("staticres100ave0",ios::app);  
    ...
    NIdRndH.SortByDat(false);
	
	TIntFltH::TIter TI = NIdRndH.BegI(); 
	...
	int num_deleted = 0;
	for(int removepercent=0; removepercent<=100; removepercent ++)
	{
		removenode=(removepercent*nodes)/100;   
		for (; num_deleted < removenode; TI++, num_deleted++) 
		{ 
			H->DelNode(TI.GetKey());
			rmvnode=TI.GetKey();
		}
		...
		for(i=0;i<maxcout;i++) //计算100次,求平均
		{
			...
			double ret=res(organism, "random", false); // remove random; 
			sum +=ret;
		
			staticfile<<organism<<" removepercent: "<<removepercent<<" j: "<<j<<" calccout: "<<i<<" res: "<<ret<<" averageres: "<<(sum/(i+1))<<endl; 
		}
		resaverage<<organism<<" rmvnode: "<<removenode<<" res: "<<(sum/maxcout)<<endl; 
